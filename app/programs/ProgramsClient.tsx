...
const PDPHCN: CourseData = {
  ...
  careerProspects: [
    'Community Nutrition Officer', 'Public Health Assistant', 'Nutrition Educator',
    'NGO / Development Programme Assistant', 'Food Safety and Quality Officer',
    'Entrepreneur in nutrition services', 'Teaching and health care professional',
  ],
  curriculum: [
    {
      year: 1, semester: 1, label: 'First Semester', totalCredits: 16,
      courses: [
        { code: 'PHCN-01', title: 'Food & Nutrition: An Introduction', credits: 4, description: 'Introduces basic principles of food, nutrients, and their roles in human health. Covers macronutrients, micronutrients, food groups, digestion, absorption, metabolism, and dietary guidelines.' },
        { code: 'PHCN-02', title: 'Therapeutic Nutrition', credits: 4, description: 'Explores the use of diet in prevention and management of diseases. Focus on diet therapy, clinical nutrition, and nutritional care process for conditions including diabetes, hypertension, obesity, and renal diseases.' },
        { code: 'PHCN-03', title: 'Food Hygiene and Sanitation', credits: 4, description: 'Covers food safety principles, hygiene practices, and sanitation techniques to prevent foodborne illnesses. Includes HACCP, Codex Alimentarius, and WASH principles.' },
        { code: 'PHCN-04', title: 'Nutrition Education', credits: 4, description: 'Introduces principles and methods of nutrition education and communication, focusing on community engagement, behavior change theories, and use of ICT and media for nutrition promotion.' },
      ],
    },
    {
      year: 1, semester: 2, label: 'Second Semester', totalCredits: 12,
      courses: [
        { code: 'PHCN-05', title: 'Community Nutrition', credits: 4, description: 'Examines nutrition problems at community level and interventions to address them. Emphasis on maternal and child nutrition, supplementation, fortification, and nutrition-sensitive agriculture.' },
        { code: 'PHCN-06', title: 'Public Health', credits: 4, description: 'Covers principles of public health with focus on nutrition-related issues and preventive health services. Includes epidemiology, health promotion, primary health care, and emerging public health issues.' },
        { code: 'PHCN-07', title: 'National & International Programmes for Public Health', credits: 4, description: 'Provides an overview of national and global nutrition and public health programs, policies, and partnerships. Covers WHO, UNICEF, FAO, WFP roles and Nigeria\'s National Policy on Food & Nutrition.' },
      ],
    },
  ],
};
...
const DPHCN: CourseData = {
  ...
  directEntryEligibility: ['Nursing', 'Physiotherapy', 'Medical Laboratory Sciences', 'Radiography', 'Related Science courses'],
  curriculum: [
    {
...
const diplomaProgrammes = [
  { id: 'pdphcn', label: 'Professional Diploma', sublabel: 'PDPHCN — 1 Year', badge: 'bg-accent-500', data: PDPHCN },
  { id: 'dphcn',  label: 'Two-Year Diploma',     sublabel: 'DPHCN — 2 Years', badge: 'bg-primary-600', data: DPHCN },
];

// ── Component ─────────────────────────────────────────────────────────────────

interface Props {
  courses: import('@/types/sanity').Course[];
}

// Map a Sanity Course to the CourseData shape expected by CourseDetail
function toCourseData(c: import('@/types/sanity').Course): CourseData {
  return {
    title: c.title,
    acronym: c.acronym,
    duration: c.duration ?? '',
    totalCredits: c.totalCredits ?? 0,
    semesters: c.curriculum?.length ?? 0,
    aim: c.aim ?? '',
    objectives: c.objectives ?? [],
    targetBeneficiaries: c.targetBeneficiaries ?? [],
    admissionText: c.admissionText ?? '',
    feeText: c.feeText,
    deliveryModes: c.deliveryModes,
    assessmentStructure: c.assessmentStructure ?? { continuousAssessment: 40, examination: 60, minimumPass: 50 },
    gradingSystem: c.gradingSystem,
    graduationRequirements: c.graduationRequirements ?? [],
    diplomaClassification: c.diplomaClassification ?? [],
    careerProspects: c.careerProspects ?? [],
    directEntryEligibility: c.directEntryEligibility,
    curriculum: c.curriculum ?? [],
  };
}

export default function ProgramsClient({ courses }: Props) {
  // Build the list of diploma programmes — prefer Sanity data, fall back to hardcoded
  const sanityProgrammes = courses.map((c, i) => ({
    id: c.acronym.toLowerCase(),
    label: c.title,
    sublabel: `${c.acronym} — ${c.duration ?? ''}`,
    badge: i === 0 ? 'bg-accent-500' : 'bg-primary-600',
    data: toCourseData(c),
  }));

  const activeProgrammes = sanityProgrammes.length > 0 ? sanityProgrammes : diplomaProgrammes;

  const [active, setActive] = useState<string>(activeProgrammes[0]?.id ?? 'pdphcn');
  const current = activeProgrammes.find((p) => p.id === active) ?? activeProgrammes[0];
        ...
        <div className="text-center mb-12">
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <h3 className="text-xl font-bold text-gray-900 mt-3">{prog.data.title}</h3>
                    <p className="text-gray-500 text-sm mt-1">({prog.data.acronym})</p>
                  </div>
                  {active === prog.id && (
                    <span className="bg-primary-600 text-white text-xs font-bold px-3 py-1 rounded-full">Viewing</span>
                  )}
                </div>
          ...
          <div className="flex items-center gap-4 mb-10">
            ...
            <div className="text-center">
              <h3 className="text-2xl font-bold text-gray-900">{current.data.title}</h3>
